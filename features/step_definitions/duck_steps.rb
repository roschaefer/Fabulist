Given(/^there is a rich duck called "Dagobert"$/) do
  dagobert = Duck.new
  proxy = TaggedObject.new(dagobert, :called => "Dagobert", :that_is => :rich)
  memorize proxy
end
...
Given(/^he has three grandchildren called "(.*?)" "(.*?)" and "(.*?)"$/) do |name1, name2, name3|
  ...
  proxy1 = TaggedObject.new(tic,  :called => name1)
  proxy2 = TaggedObject.new(tric, :called => name2)
  proxy3 = TaggedObject.new(trac, :called => name3)
  memorize proxy1
  memorize proxy2
  memorize proxy3
end
